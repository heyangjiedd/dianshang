...
export default class Index extends React.Component{
    ...
    _renderBanner(){
        if(this.state.newGoods.length!=0){
            if(this.state.newGoods.length==1){
                return (<img src={'http://116.62.119.165/fileServer/images/'+this.state.newGoods[0].showUrl}/>)
            }else{
                return (
    ...
    bundleBanner(){
        ...
        $.ajax({
            type: "get",
            url: "http://116.62.119.165/shop-portal/swagger/api/areBanner",
            async:true,
            data: {
...
//                  }
                    var bannerArray=[];
                    bannerArray.push(response.data);
