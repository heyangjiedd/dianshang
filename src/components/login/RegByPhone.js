...
export default class RegByPhone extends React.Component{
    ...
    saveNickName(){
        ...
        $.ajax({
            type: "get",
            url: "http://116.62.119.165/shop-portal/swagger/api/validRegisterCode",//请求的路径
            async:true,
            data: {
            ...
            success: function(response, textStatus){
                ...
                if(response.code==1){
                    hashHistory.push({
                        pathname: '/LoginByPhoneSetPass',
                        search: '?phone='+_this.state.value,
                        state: { phone: _this.state.value }
                    })
                }else{
